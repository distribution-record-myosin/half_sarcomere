...
!  real(8) :: dictp_2d(dict_psz, dictp_2d_xsz, dictp_2d_shift_sz)
  ...
  x_L_value=7.0
  x_S_value=-3.0
  call index_determine(index1, x_L_value, real(dict_2d_xlowb,kind=8), real(dict_2d_xupb,kind=8), dict_2d_xsz)
  call index_determine(index2, x_S_value, real(dict_2d_slowb,kind=8), real(dict_2d_supb,kind=8), dict_2d_shift_sz)  

  pre_count=0
  pos_count=0
  if(x_L_value < x_barrier) then
    do i=1,dict_psz
      if(dict_2d(i,index1,index2)>x_barrier) then
  ...
  write(*,*) "x_L,x_S:",x_L_value, x_S_value
  if(x_L_value < x_barrier) then
    write(*,*) "pos_count:", pos_count
    write(*,*) "rate:", pos_count/real(dict_psz,kind=8)    
  else 
    write(*,*) "pre_count:", pre_count  
    write(*,*) "rate:", pre_count/real(dict_psz,kind=8)        
  end if
...
contains
  ...
  !C changed to store absolute values.
